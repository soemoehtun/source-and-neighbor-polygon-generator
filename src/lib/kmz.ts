...
/** Each source KMZ has exactly one Polygon, plus its source and matched-neighbour markers. */
export function buildSiteKml(result: SourceResult, style: SiteStyle): string {
  const sourcePoint =
  ...
  const neighbours = result.neighbors
    .map(
      (n) =>
  ...
  return (
    ...
    `${sourcePoint}<Folder><name>Neighbours (${result.neighbors.length})</name>${neighbours}</Folder>` +
    `${polygon}</Folder></Document></kml>`
  );
}
...
async function siteKmzBlob(result: SourceResult, style: SiteStyle): Promise<Blob> {
  const kmz = new JSZip();
  kmz.file('doc.kml', buildSiteKml(result, style));
...
export async function exportSiteKmz(result: SourceResult, index: number, style?: SiteStyle) {
  if (result.hull.length < 3) return;
  triggerDownload(await siteKmzBlob(result, style ?? defaultSiteStyle(index)), `${safeFilename(result.source.site)}.kmz`);
}
...
export async function exportKmzBundle(
  ...
  for (const { result, index } of eligible) {
    ...
    names.add(filename);
    bundle.file(filename, await siteKmzBlob(result, styleFor(index)));
  }
  triggerDownload(
    await bundle.generateAsync({ type: 'blob', compression: 'DEFLATE', compressionOptions: { level: 6 } }),
    'source_site_kmz_files.zip',
  );
}